...
export default function SoundControls({ className = '' }: SoundControlsProps) {
  ...
  return (
    <div className={`flex md:flex-row flex-col items-end md:items-center gap-2 md:gap-3 ${className}`}>
      {enabled && (
      ...
      <button
        ...
        aria-label={enabled ? 'Disable LCARS sounds' : 'Enable LCARS sounds'}
      >
        {enabled ? (
          <>
            ...
            <span className="hidden md:inline">Sounds On</span>
          </>
        ) : (
          <>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
            ...
            <span className="hidden md:inline">Sounds Off</span>
          </>
        )}
      </button>
    </div>
